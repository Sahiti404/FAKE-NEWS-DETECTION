...
def predict_fake_or_true_(title, text, model, vectorizer, selector):
    ...
    fake_probability = prediction_probabilities[0][1]
    
   
    predicted_label = "FAKE" if fake_probability * 100 > 50 else "TRUE"
    
   
    fakeness_percentage = fake_probability * 100
    
    return predicted_label, fakeness_percentage
